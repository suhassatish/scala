...
object Anagrams {
  ...
  def wordOccurrences1(w: Word): Occurrences = {
    w.toLowerCase.toCharArray.groupBy(chr => chr).map {
      case (c: Char, c_arr: Array[Char]) => c -> c_arr.length
    }.toList.sorted
  }

  def wordOccurrences(w: Word): Occurrences = {
    val unsorted = w.toLowerCase.groupBy(identity).map {
      case (c: Char, cs: String) => (c, cs.length)
    }
...
//  {
//    s.map(word => wordOccurrences(word.replaceAll("""[\p{Punct}||\s+]""", "")))
//  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y foldLeft SortedMap[Char,Int]() ++ x){ case (map, (ch, tm)) => {
      val newTm = map(ch) - tm
      if (newTm != 0) map updated (ch, newTm)  //Map[ch] = newTm
      else map - ch  //removes key ch from Map
    } }.toList
  }
