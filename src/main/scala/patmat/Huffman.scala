...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    ...
    //chars.foldLeft(Map[Char, Int]())(incr).iterator.toList  //above is same syntax as this line where starting operand
    // is an instance of empty map.
    // More foldLeft examples here for reference with lists and tuples
   ...
   */
  def until(singlton: List[CodeTree] => Boolean, combne: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    if (singlton(trees)) trees
    else until(singlton, combne)(combne(trees))
  }
