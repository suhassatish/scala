...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(
      ...
      columnNames.tail.map { item => StructField(item, DoubleType, nullable = false) }
    )
  ...
  def row(line: List[String]): Row =
    Row(line.head :: line.tail.map(_.toDouble) : _*)  // _* flattens the tail
  ...
  def classifiedColumnsOOM(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primary = List[Column]()
    val working = List[Column]()
    val leisure = List[Column]()
    _classifiedColumns(columnNames, primary, working, leisure)
  }

  def _classifiedColumns(columnNames: List[String], primary: List[Column], working: List[Column], leisure: List[Column]) : (List[Column], List[Column], List[Column]) = {
    columnNames match {
      ...
      case x :: xs =>
        if (x.startsWith("t01") || x.startsWith("t03") || x.startsWith("t11") || x.startsWith("t1801") || x.startsWith("t1803"))
          _classifiedColumns(columnNames, new Column(x) :: primary, working, leisure)

        else if (x.startsWith("t05") || x.startsWith("t1805"))
          _classifiedColumns(xs, primary, new Column(x)::working, leisure)
          ...
          _classifiedColumns(xs, primary, working,  new Column(x)::leisure)

        else
          _classifiedColumns(xs, primary, working, leisure)
    }
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column =
      when('telfs >= 1.0 && 'telfs < 3.0, "working")
        ...
        .as("working")  // column name in new table

    val sexProjection: Column = when(df("tesex") === 1, "male").otherwise("female").as("sex")  // notice 2 different formats
    ...
    // Hint: don’t forget to convert the value to hours (from minutes)
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    // timeUsageSummaryDf.map(r => TimeUsageRow(r.getAs("working"), r.getAs("sex"), r.getAs("age"), r.getAs("primaryNeeds"), r.getAs("work"), r.getAs("other")))  //this works but is cumbersome
